...
public class ZonePresenceTracker : MonoBehaviourPunCallbacks
{
    ...
    private void Update()
    {
        ...
        foreach (Player player in PhotonNetwork.CurrentRoom.Players.Values)
        {
            ...
            if (view == null)
                continue;
            samples.Add((team, manager.TryGetZoneAt(MeasuredPosition(view), out int zone) ? zone : -1));
        }
    ...
    /// map from the death spot to the spawn, through other zones on the way.</summary>
    private static Vector3 MeasuredPosition(PhotonView view)
    {
        if (!view.IsMine && view.TryGetComponent(out PlayerNetSync sync))
